...
const AppLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-cream flex">
      ...
      {/* Search Modal */}
      {searchOpen && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-20" onClick={() => setSearchOpen(false)}>
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
          <div ref={searchRef} className="relative w-full max-w-2xl mx-4 bg-white rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-4">
                  {searchResults.bookings?.length > 0 && (
                    <div>
                      ...
                      {searchResults.bookings.slice(0, 5).map((b, i) => (
                        <button key={i} onClick={() => handleSearchNavigate('/bookings')} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 text-left">
                  ...
                  {searchResults.services?.length > 0 && (
                    <div>
                      ...
                      {searchResults.services.slice(0, 5).map((s, i) => (
                        <button key={i} onClick={() => handleSearchNavigate('/services')} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 text-left">
                          <Scissors className="w-5 h-5 text-purple-500" />
                          <div className="flex-1">
                            <p className="font-medium">{s.name}</p>
                            <p className="text-sm text-gray-500">£{(s.price_pence / 100).toFixed(2)}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {searchResults.customers?.length > 0 && (
                    <div>
                      ...
                      {searchResults.customers.slice(0, 5).map((c, i) => (
                        <button key={i} onClick={() => handleSearchNavigate('/bookings')} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 text-left">
